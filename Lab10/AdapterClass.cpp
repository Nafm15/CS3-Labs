...
public:
  LegacyRectangle(int topLeftX,int topLeftY,int bottomRightX,int bottomRightY):topLeftX_(top\
											 LeftX),topLeftY_(topLeftY),bottomRightX_(bottomRightX),bottomRightY_(bottomRightY){}
...
int main(){
  ...
  SquareAdapter *sq = new SquareAdapter(size);
  cout << "Cuurent Size : " << sq -> size() << endl;

  int newSize = 0;
  cout << "Enter New Size of the square : ";
  cin >> newSize;
  cout << endl;
  cout << "Resizing... ... ..." << endl;
  sq -> resize(newSize); //square -> resize(newSize);
  sq -> draw();
}
